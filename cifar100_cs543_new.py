...
class BaseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        '''
        x = F.relu(self.conv1(x)) # 32 x 32
        x = self.bn1(x)
        x = F.relu(self.conv2(x)) # 32 x 32
        x = self.bn2(x)
        x = F.relu(self.conv3(x))
        x = self.bn3(x)
        x = self.pool1(x) # 16 x 16

        x = F.relu(self.conv4(x)) # 16 x 16
        x = self.bn4(x)
        x = F.relu(self.conv5(x)) # 16 x 16
        x = self.bn5(x)
        x = F.relu(self.conv6(x)) # 16 x 16
        x = self.bn6(x)
        x = self.pool2(x) # 8 x 8
        '''

        # VGG net (version A): half of original size
        x = nn.Conv2d(3, 64, kernel_size = 3, padding = 1)(x)
        x = nn.BatchNorm2d(64)(x)
        x = nn.ReLU(inplace = True)(x)
        x = nn.MaxPool2d(kernel_size = 2, stride = 2)(x)

        x = nn.Conv2d(64, 128, kernel_size = 3, padding = 1)(x)
        x = nn.BatchNorm2d(128)(x)
        x = nn.ReLU(inplace = True)(x)
        x = nn.MaxPool2d(kernel_size = 2, stride = 2)(x)

        x = nn.Conv2d(128, 256, kernel_size = 3, padding = 1)(x)
        ...
        x = nn.Conv2d(512, 512, kernel_size = 3, padding = 1)(x)
...
optimizer = optim.SGD(net.parameters(), lr=0.012, momentum=0.95)
...
for epoch in range(EPOCHS):  # loop over the dataset multiple times
    ...
    train_acc, train_class_acc = calculate_val_accuracy(trainloader, IS_GPU)
    print('Accuracy of the network on the training images: %d %%' % (train_acc))
